...
export default function Analytics() {
  ...
  const monthly = useMemo(() => getMonthlySummary(transactions), [transactions]);
                    ...
                    return (
                      <div key={item.category} className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground flex-1 truncate">
                          {config?.emoji} {config?.label || item.category}
                        </span>
                  ...
                  <div key={i} className="rounded-xl bg-card border border-border/50 p-4 space-y-2">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {config?.emoji} {config?.label} · Last: {item.lastDate}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-border/30">
                      <span className="text-xs text-muted-foreground">
                        Avg: <span className={`font-medium ${item.type === "income" ? "text-income" : "text-expense"}`}>
                          ${item.avgAmount.toFixed(2)}
                        </span>
                      </span>
                      <span className="text-xs text-muted-foreground">
                        Total: <span className={`font-medium ${item.type === "income" ? "text-income" : "text-expense"}`}>
